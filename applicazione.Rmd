...
plays <- dataset_plays %>%
  ...
  mutate(total_played = n())%>%
  ungroup()%>%
  filter(total_played>50)%>%
  left_join(games , by = c("game_id"="id"))%>%
  select(game_id, period, player, turn, card_id, name, mana, total_played)
...
nodes <- filtered_plays %>%
  ...
  summarize(avg_turn = mean(turn), 
            total_played = mean(total_played), 
            mana=mean(mana) )%>%
  select(name,mana,avg_turn, total_played)%>%
  arrange(avg_turn)
...
for( i in (2:total)){
  ...
  if (row1$game_id == row2$game_id) {
    ...
    card_name1[[i]]<- row1$name
    card_name2[[i]]<- row2$name
