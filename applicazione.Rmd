...
nodes <- filtered_plays %>%
  ...
  summarize(avg_turn = mean(turn), 
            mana=mean(mana),
            winrate=-1/log(sum(result)/n()) )%>% # -1/log(0.8) > -1/log(0.5) > -1/log(0.3)
  ungroup()%>%
...
lay = create_layout(g, layout = "with_kk")
...
```{r card network plotting}
ggraph(g, layout = lay) + 
  geom_edge_link(aes(alpha = weight)) + # edge link geom, mapping alpha to weight
  geom_node_point(aes()) +     # node point geom, mapping size to pagerank centrality
  theme_graph(base_family="sans")
```

Let's analyze this network!!
First of all...which are the most versatile cards? They are such cards which lies in lot of path (games).
```{r betweenness}
...
giocate_corrette <- nodes %>%
  ...
  select(name, mana.x, winrate.x, card_2, mana.y,winrate.y)%>%
  distinct()

knitr::kable(
  head(giocate_corrette %>% arrange(-(winrate.y-winrate.x)),20), 
  caption = "Most important cards (using power)"
)
...
Combos are usually good plays...is it real?